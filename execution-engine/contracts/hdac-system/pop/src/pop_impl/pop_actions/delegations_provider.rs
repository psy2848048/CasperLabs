...
impl ProofOfProfessionContract {
    ...
    pub fn write_delegations(&self, delegations: &Delegations) {
    ...
    pub fn read_delegation_stat(&self) -> Result<DelegationStat> {
    ...
    pub fn read_delegation_user_stat(&self) -> Result<DelegationStat> {
        let mut delegation_stat = BTreeMap::new();
        for (name, _) in runtime::list_named_keys() {
            let mut split_name = name.split('_');
            if Some("d") != split_name.next() {
    ...
    pub fn get_sorted_delegation_stat(
        &self,
