...
fn should_run_successful_step() {
    ...
    const ACCOUNT_2_BOND_AMOUNT: u64 = BIGSUN_TO_HDAC;
    ...
    let genesis_total_supply: U512 =
        U512::from(DEFAULT_ACCOUNT_INITIAL_BALANCE + GENESIS_VALIDATOR_STAKE) * 2;

    // ACCOUNT_2 bond additionally
    let bond_request = ExecuteRequestBuilder::standard(
        ACCOUNT_2_ADDR,
        CONTRACT_POS_VOTE,
        (String::from(METHOD_BOND), U512::from(ACCOUNT_2_BOND_AMOUNT)),
    )
    .build();
    ...
    assert!(account_1_balance_before < account_1_balance_after + *DEFAULT_PAYMENT);
